class ProjectDecorator < Draper::Decorator
  ...
  # note we are using the facebook version because it is
  # the only one big enough to fill the project boxes as they
  ...
  def display_pledged
    number_to_currency source.pledged, unit: '$', precision: 0, delimiter: ','
  end

  def display_goal
    number_to_currency source.goal, unit: '$', precision: 0, delimiter: ','
  end
