CATARSE.ProjectsShowView = Backbone.View.extend({
  ...
  backers: function() {
    ...
    this.backersView = new this.BackersView({
      ...
      collection: new CATARSE.Backers({url: '/projects/' + this.project.id + '/backers'}),
...
$( document ).ready(function() {
  var total = 0,
      $count = $('#shareme'),
      updateTotal = function(selector) {
        var $this = $(selector),
            total = parseFloat($count.data('total')),
            count = parseFloat($this.text());

        total = total + count;
        $count.data('total', total);
        $count.html(total + '<span>shares</span>');
      };
  $('.share-twitter').sharrre({
    ...
    enableHover: false,
    enableTracking: true,
    buttons: { twitter: {via: 'endlesscrowds'}},
    render: function() {
      updateTotal('#twitter .count');
    },
  ...
  var setJSON = function(data) {
    var selector = '.share-' + data.type + ' .count';
...
//extra =
// findmehere
  $.each(["googleplus"], function(i, service) {
      var shareButtons = $('.share-' + service),
          blogUrl, path, url;

      if (shareButtons.length) {
        blogUrl = shareButtons.data('url'),
        path = '/?url=' + encodeURIComponent(blogUrl.match(/\/$/) ? blogUrl : blogUrl + '/') + '&type=' + service,
        url = 'http://endlesscrowds-share-counts.herokuapp.com' + path,
        $.ajax({url: url, dataType: 'jsonp', jsonpCallback: 'setJSON', success: setJSON});
      }
  });
});
